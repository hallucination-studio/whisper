...
pub(super) fn query_raw(path: &Path, limit: usize) -> Result<Vec<RawFact>, HostError> {
    ...
    for row in rows {
        ...
        let key_epoch = u16::from_be_bytes(
            key_epoch
                .try_into()
                .map_err(|_| HostError::message("persisted key epoch is invalid"))?,
        );
        let boot_generation = u32::from_be_bytes(
            boot_generation
                .try_into()
                .map_err(|_| HostError::message("persisted boot generation is invalid"))?,
        );
        ...
        facts.push(RawFact {
            ...
            key_epoch: KeyEpoch::new(key_epoch)
                .ok_or_else(|| HostError::message("persisted key epoch is invalid"))?,
            boot_generation: BootGeneration::new(boot_generation)
                .ok_or_else(|| HostError::message("persisted boot generation is invalid"))?,
            message_sequence: MessageSequence::new(message_sequence)
                .ok_or_else(|| HostError::message("persisted message sequence is invalid"))?,
        ...
        losses.push(RawLoss {
            ...
            .map(|boot| {
                BootGeneration::new(boot)
                    .ok_or_else(|| HostError::message("persisted loss boot is invalid"))
            })
            .transpose()?,
            first_sequence: decode_optional_sequence(first)?.map(|sequence| {
                MessageSequence::new(sequence).expect("decoded sequence is nonzero")
            }),
            last_sequence: decode_optional_sequence(last)?.map(|sequence| {
                MessageSequence::new(sequence).expect("decoded sequence is nonzero")
            }),
        });
    }
...
fn decode_optional_u32(
    ...
    bytes
        .map(|bytes| {
            let bytes: [u8; 4] = bytes.try_into().map_err(|_| HostError::message(invalid))?;
...
fn decode_optional_sequence(bytes: Option<Vec<u8>>) -> Result<Option<u64>, HostError> {
    bytes
        .map(|bytes| {
            ...
            if sequence == 0 {
                return Err(HostError::message("persisted loss sequence is invalid"));
            }
            Ok(sequence)
        })
        .transpose()
